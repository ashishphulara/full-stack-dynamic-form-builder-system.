    ...
    for (const field of schema.fields) {
        ...
        if (field.type === "multi-select" && Array.isArray(value)) {
            if (rules.minSelected && value.length < rules.minSelected) {
                errors[field.name] = `${field.label} must have at least ${rules.minSelected} options selected`;
            }
            if (rules.maxSelected && value.length > rules.maxSelected) {
                errors[field.name] = `${field.label} must have at most ${rules.maxSelected} options selected`;
            }
        }
    }
    return errors;
